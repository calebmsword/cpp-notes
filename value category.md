...
In C++11, the standards committee wanted to categorize expressions whose evaluation determines an object's **identity**. These expressions are said to "have identity". Expressions have identity if their value is determined by calculating a specific address in memory, act as an umabigious identifier for some specific object, or if they are lvalues. There are many types of expressions with identity:

 - expressions which calculate specific memory addresses:
 ...
 - expressions that are umabigious identifiers for an object:
